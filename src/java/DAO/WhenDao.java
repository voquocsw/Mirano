...
// 1: san sang
// 2: da huy
public class WhenDao extends DBContext {

    public int countWhenActive() {
        try {
            String sql = "select count(WhenId) as countWhen from Show where status = 1";
            PreparedStatement stm = connection.prepareCall(sql);
            ResultSet rs = stm.executeQuery();
            if (rs.next()) {
                int count = rs.getInt("countWhen");
                return count;
            }
            return 0;
        } catch (SQLException ex) {
            Logger.getLogger(WhenDao.class.getName()).log(Level.SEVERE, null, ex);
            return 0;
        }
    }
...
//    }
    ...
    public int updateStatusWhen(int status) {
        try {
            String sql = "UPDATE [dbo].[Show]\n"
                    + "   SET [status] = 0\n"
                    + " WHERE showId = ?";
    ...
    public List<Show> getShowByCondition(String date, String order, int page, int pageSize) {
    ...
    public When findShow(int slot, Date date) {
        try {
            String sql = "  select s.showId, s.showDate, s.slotId, sl.[Time] as slotTime, s.roomId, r.[Name] as roomName,\n"
                    + "  s.filmId, f.Title, s.status from show s left join Films f on s.filmId = f.FilmID \n"
                    + " left join Slot sl  on s.slotId = sl.slotId\n"
